...
We used [fastai](http://fast.ai) which is built on type of pytorch and makes it easy to build CNNs for common image recognition tasks (as well as other deep learning tasks such as NLP)^[If you are interested in neural nets, I highly recommend fast.ai's MOOC.]. Given this framework (and the power of Google Colab's GPUs), I was able to get a model to 98% accuracy in just a few minutes. The issue I was running into was that I hadn't built my own validation set, so I relied on fastai's ability to randomly create one for me. With just 1 second between images, I was afraid our validation set was too similar to the training set and wouldn't accurately detect an overfit model. I spent the next few hours creating my own validation set and tuning the model and found that, while the model was ok (~70% accurate), it was not doing as well as I had hoped.
...
1. Add in a screen when the app started in order to allow you to align your webcam.
2. Plot some feedback at the end of your study session so that you can see when you failed to pay attention.
3. Gather some more data and continue to improve the model.
